		...
		<div className='flexible vertical float-panel gap'>
			...
			<div className='flexible vertical'>
				...
				<div className='flexible'>
					<input type='checkbox' checked={vectorVisibility} onChange={e => setVectorVisibility(e.target.checked)} disabled={geojson ? false : true} />
					Vector layer
				</div>
			</div>
			...
			<div className='flexible vertical'>
				...
				<div className='flexible wide'>
					<input type='date' value={startDate} disabled={geojson ? false : true} onChange={e => {
					...
					-
					<input type='date' value={endDate} disabled={geojson ? false : true} onChange={e => {
						let value = e.target.value;
						if (value <= startDate){
			...
			}} disabled={geojson ? false : true}>Show layer</button>

			<a href={thumbnailUrl} download target='_blank'>
				<button disabled={thumbnailUrl ? false : true} style={{ width: '100%' }}>Download image </button>
			</a>
		</div>	
	)
}