...
// LAI function
export function lai(samples) {
  const sample = samples[0];
  const b03_norm = normalize(sample.B03, 0, 0.253061520471542);
  const b04_norm = normalize(sample.B04, 0, 0.290393577911328);
  const b05_norm = normalize(sample.B05, 0, 0.305398915248555);
  const b06_norm = normalize(sample.B06, 0.006637972542253, 0.608900395797889);
  const b07_norm = normalize(sample.B07, 0.013972727018939, 0.753827384322927);
  const b8a_norm = normalize(sample.B8A, 0.026690138082061, 0.782011770669178);
  const b11_norm = normalize(sample.B11, 0.016388074192258, 0.493761397883092);
  const b12_norm = normalize(sample.B12, 0, 0.493025984460231);
  const viewZen_norm = normalize(Math.cos(sample.viewZenithMean * degToRad), 0.918595400582046, 1);
  const sunZen_norm  = normalize(Math.cos(sample.sunZenithAngles * degToRad), 0.342022871159208, 0.936206429175402);
  const relAzim_norm = Math.cos((sample.sunAzimuthAngles - sample.viewAzimuthMean) * degToRad)
